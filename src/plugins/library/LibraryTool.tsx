...
export default function LibraryTool() {
  ...
  const changeSort = (field: SortField) => setFilter((value) => {
    if (!value.sort || value.sort.field !== field) return { ...value, sort: { field, direction: 'asc' } };
    if (value.sort.direction === 'asc') return { ...value, sort: { field, direction: 'desc' } };
  ...
  <section style={{ padding: 16 }}>
    ...
    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
      <input aria-label="搜索图书" placeholder="搜索标题、格式、分类、描述" value={filter.query} onChange={(event) => setFilter({ ...filter, query: event.target.value })} />
      ...
      <select aria-label="格式筛选" value={filter.format ?? ''} onChange={(event) => setFilter({ ...filter, format: event.target.value || null })}><option value="">所有格式</option>{formats.map((format) => <option key={format}>{format}</option>)}</select>
      <select aria-label="分类筛选" value={filter.type ?? ''} onChange={(event) => setFilter({ ...filter, type: event.target.value || null })}><option value="">所有分类</option>{types.map((type) => <option key={type}>{type}</option>)}</select>
...
function ImportDialog({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  const [target, setTarget] = useState<boolean | null>(null); const targetRef = useRef<boolean | null>(null); const onDoneRef = useRef(onDone); const [results, setResults] = useState<Array<{ source: string; status: string; message: string }>>([]); const [importing, setImporting] = useState(false);
...
function SortHeader({ field, label, sort, onSort }: { field: SortField; label: string; sort: LibraryFilter['sort']; onSort: (field: SortField) => void }) {
  ...
  const indicator = rule ? (rule.direction === 'asc' ? '↑' : '↓') : '＝';
  const order = rule ? (rule.direction === 'asc' ? '升序' : '降序') : '未排序';
  return <button onClick={() => onSort(field)} aria-sort={rule ? (rule.direction === 'asc' ? 'ascending' : 'descending') : 'none'} title={`${label}：${order}。依次切换为升序、降序、未排序`} style={{ border: 0, background: 'transparent', textAlign: 'left', cursor: 'pointer' }}>{label} <span aria-hidden="true">{indicator}</span></button>;
}
