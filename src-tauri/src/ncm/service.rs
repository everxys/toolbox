// NCM implementation; Tauri protocol entry points remain in commands::ncm.
use tauri::{AppHandle, Manager};
use serde::{Deserialize, Serialize};
use std::path::PathBuf;
use super::{auth, client, download, repository};
...
fn download_file_name(name: &str, artists: &[String], url: &str) -> String {
    download::download_file_name(name, artists, url)
}
...
#[cfg(test)]
mod tests {
    use super::{download_file_name, extract_music_u_cookie, nickname_from_login_status, player_cookie, qr_check_path, LoginState, QrCreateResp};
    use super::download::ensure_audio_response;
    use super::auth::{clear_login_state, require_login_cookie};
    use std::sync::Mutex;
...
pub async fn ncm_download(app: AppHandle, id: i64, name: String, artists: Vec<String>, level: Option<String>) -> Result<serde_json::Value, String> {
    ...
    download::stream_to_file(ncm_client(), &url, &path).await?;
    let file_path = path.to_string_lossy().to_string();
    let app_for_db = app.clone();
    tokio::task::spawn_blocking(move || repository::record_download(&app_for_db, id, Some(&file_path)))
        .await
        .map_err(|e| e.to_string())?
        .map_err(|e| e.to_string())?;
