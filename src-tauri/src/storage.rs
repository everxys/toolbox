...
#[cfg(test)]
mod tests {
    use super::{validate_database, DATABASE_FILE_NAME};
    ...
    #[test]
    fn integrity_check_accepts_sqlite_and_rejects_synthetic_corruption() {
        let root = std::env::temp_dir().join(format!("toolbox-storage-test-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        fs::create_dir_all(&root).unwrap();
        let valid = root.join("toolbox.db");
        rusqlite::Connection::open(&valid).unwrap().execute("CREATE TABLE sample (id INTEGER)", []).unwrap();
        assert!(validate_database(&valid).is_ok());
        let invalid = root.join("damaged.db");
        fs::write(&invalid, b"not a sqlite database").unwrap();
        assert!(validate_database(&invalid).is_err());
